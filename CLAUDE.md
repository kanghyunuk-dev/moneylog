...
- [ ] **②단계 진행 중** — 카테고리 조회 + 거래(Transaction) CRUD 백엔드, 사이드바 레이아웃(`Sidebar`/`Layout`)+거래내역 화면(`TransactionsPage`, 목록/등록/수정/삭제) 프론트까지 `develop`에 병합 완료(v0.3.0). 이어서 `feature/budgets`, `feature/dashboard` 남음. 판단 근거는 `docs/decisions.md` 참고

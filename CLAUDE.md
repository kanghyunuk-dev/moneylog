...
- [ ] **②단계 진행 중** — 카테고리 조회 + 거래(Transaction) CRUD, 사이드바 레이아웃+거래내역 화면 `develop`에 병합 완료(v0.3.0). `feature/budgets`(예산 CRUD + 카테고리별 지출 집계 `spentAmount` + `BudgetsPage` 화면)도 백엔드/프론트 전부 완성, 브라우저 실동작 검증 완료(2026-09-10, 커밋 전). 이어서 `feature/dashboard` 남음. 판단 근거는 `docs/decisions.md` 참고

...
- [x] ①단계 7/8단계(마이페이지) 백엔드 진행 중 — `GET/PUT /api/users/me`, `PUT /api/users/me/password` 작성 및 Postman 실동작 검증 완료(닉네임/비밀번호 변경까지 확인). 프론트(`MyPage.jsx`)는 미착수
- [ ] ①단계 나머지(7단계 프론트, 8단계 소프트삭제) 순서대로 진행
- [ ] ②단계 이후 순서대로 진행

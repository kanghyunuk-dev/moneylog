...
- **①단계 토큰 저장 — localStorage로 시작 후 httpOnly+CSRF로 전환(2026-08-26 완료)**: 실무 표준은 httpOnly 쿠키+CSRF 토큰(XSS에 더 안전)이지만, 쿠키 속성·CSRF·CORS credentials까지 한번에 얹으면 로그인 실패 시 원인 구분이 어려워짐 — JWT 흐름을 직접 짜보며 체화하는 초반 단계는 localStorage로 격리해서 익힘. 전환 시점은 "①단계 완주 후"가 아니라 로그인/회원가입 실동작 검증이 끝난 6/8단계 직후로 앞당김 — 두 방식을 비교 학습하려는 목적이라 7~8단계(마이페이지, 소프트삭제)를 만들기 전에 인증 흐름 자체를 먼저 실무 표준으로 맞춰두는 게 재작업이 적음. 실제 전환 내용은 `docs/decisions.md`의 "백엔드/프론트엔드 구현 판단" 참고.
...
- **에러 응답에 `errorCode` 필드 추가(2026-08-28)**: 같은 401 안에 "토큰 문제(재발급하면 해결)"와 "도메인 검증 실패(비밀번호 불일치, 재발급해도 안 풀림)"가 섞여 있어 `authFetch`가 후자에도 불필요한 재발급을 시도하던 문제 — 401을 403으로 바꾸는 대신(RFC 7235상 401이 맞는 코드), `ErrorResponse`에 `errorCode`(예: `"TOKEN_INVALID"`)를 추가해 같은 401 안에서 세분화. 기존 `new ErrorResponse(message)` 호출부가 안 깨지도록 `message`만 받는 보조 생성자를 둠. 겪은 문제와 원인(`AuthController.me()`의 바디 없는 401)은 `docs/troubleshooting.md` 참고.
...
- **탈퇴 회원 이메일 재사용은 30일 유예 기간 동안 차단(2026-08-30)**: 즉시 허용하려면 `users.email` UNIQUE 제약을 부분 유니크로 바꿔야 해 변경 폭이 커짐 — 기존 유예 기간 설계와 맞물리는 "차단"으로 확정. `register()` 로직은 그대로 두고 에러 메시지만 "이미 가입 된 이메일 입니다" → "사용할 수 없는 이메일 입니다"로 정정(탈퇴 계정에 "가입되어 있다"는 표현이 부정확했음).
...
- **`#root`의 폭 제한(1126px)+테두리를 `.auth-layout` 클래스로 분리(2026-09-04)**: 기존엔 `#root`에 전역으로 걸려 있어 모든 화면이 좁고 가운데 정렬된 카드 형태였음. 사이드바(`Sidebar`+`Layout`) 도입 시 이 제한이 그대로면 사이드바+콘텐츠가 화면 전체 너비를 못 씀 — 로그인/회원가입(비로그인, 좁은 카드 UI가 맞음)과 로그인 후 화면(사이드바+전체 너비)의 레이아웃 요구가 다르다고 판단해, `#root`는 `flex`/`min-height`만 남기고 폭 제한은 `LoginPage`/`SignupPage` 전용 `.auth-layout` wrapper로 옮김.
